    ...
    if request.method=='POST':
        ...
        if validVoter:
            if not Registered:
                ...
                otp_number = otp_gen()
                voter.phone_number= phonenumber
...
def result(request):
    time = get_vote_auth()
    models.VoteAuth.resultCalculated = False
    if time[0].end<datetime.datetime.now(datetime.timezone.utc):
        if request.method == "GET":
            ...
            if not resultCalculated:
                ...
                vote_auth.save() 
                models.Candidate.count = 0
                list_of_votes = models.Vote.objects.all()
                list_of_candidates = models.Candidate.objects.all()
...
def forgot_password(request):
    if request.method == 'POST':
        username = request.POST.get('username')
        try:
            voter = models.VoterList.objects.get(username=username)
        except ObjectDoesNotExist:
            return render(request, 'registration/forgot_password.html', {'error': 'Invalid username'})
        
        otp_number = otp_gen()
        voter.otp = otp_number
        voter.save()
        # Send OTP to the user's phone number
        sms(voter.ph_country_code + voter.phone_number, f'Your OTP is {otp_number}')
        
        # Pass necessary data to the reset password page
        context = {
            'username': username,
            'country_code': voter.ph_country_code,
            'starred': "*******" + str(voter.phone_number)[-3:]
...
def reset_password(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'registration/reset_password.html', {'error': 'Invalid username'})

        try:
            voter = models.VoterList.objects.get(username=username)
        except models.VoterList.DoesNotExist:
            return render(request, 'registration/reset_password.html', {'error': 'Invalid username'})
        print(voter.otp)
        
        if int(otp) != int(voter.otp):
            return render(request, 'registration/reset_password.html', {'error': 'Invalid OTP'})
